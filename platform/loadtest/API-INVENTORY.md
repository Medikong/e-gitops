...
| 상태 변경 부하 | DB, outbox, Kafka 등에 상태를 남기는 사용자 요청 | 독립 결정론적 주소 범위와 idempotency key를 사용 |
...
`/healthz`, `/readyz`, `/metrics`는 모든 서비스에서 준비 상태 또는 관측 전용이다. readiness 확인 요청, 인증 setup, 데이터셋 준비 시간은 공식 latency에 포함하지 않는다.

profile에서 `addressAccess: read`는 seed에서 계산한 존재 ID를 반복 조회해도 된다는 뜻이고, `addressAccess: write`는 해당 시행에만 쓸 비중복 범위를 배정한다는 뜻이다. HTTP 메서드나 `상태 변경 부하` 분류만으로 주소 소모 여부를 추측하지 않는다. 예를 들어 Auth email sign-in은 세션을 만들지만 사용자 주소는 `read`이고, Payment approve·fail과 Web checkout confirm은 `write`다.
...
| `POST /api/v1/auth/signins/email` | 상태 변경 부하 | 측정 | 30% | seed에서 유도한 테스트 계정으로 정상 로그인하며 세션·쿠키 생성 비용을 포함한다. |
...
| `POST /api/v1/auth/registrations*` | 이번 측정 제외 | 제외 | - | 가입 수명주기와 검증 절차가 별도다. |
...
| 요청 경로 | 분류 | 측정 여부 | profile 비중 | 인증·상태 변화 |
| --- | --- | --- | ---: | --- |
| `GET /api/v1/users/me/profile` | 조회 부하 | 측정 | 75% | 결정론적 `userId`에 대응하는 k6 메모리 Bearer session을 사용한다. |
...
| `GET /drops/{dropId}` | 조회 부하 | 측정 | 40% | seed에서 계산한 현재 조회 가능한 ID만 쓴다. 응답 안의 상품은 하나의 drop에 속한다. |
...
| `GET /api/v1/users/me/coupons/{userCouponId}` | 조회 부하 | 측정 | 25% | seed에서 계산한 사용자와 쿠폰 소유 관계만 쓴다. |
| `POST /api/v1/coupon-campaigns/{campaignId}/claims` | 상태 변경 부하 | 측정 | 25% | 같은 candidate의 `couponClaim` 범위와 idempotency key를 사용한다. |
| `POST /api/v1/coupon-code-redemptions` | 상태 변경 부하 | 제외 | - | 사용자 코드 조회 API가 없고 DB에는 원문이 아닌 해시만 저장하므로, 코드 원문이나 해시 키를 loadtest Secret으로 전달하지 않는다. |
...
| `POST /v1/drops/{dropId}/views` | 상태 변경 부하 | 측정 | 15% | 비중복 결정론적 주소로 조회 이벤트와 누적 집계를 만든다. |
...
| 요청 경로 | 분류 | 측정 여부 | profile 비중 | 인증·상태 변화 |
| --- | --- | --- | ---: | --- |
| `GET /orders/{orderId}` | 조회 부하 | 측정 | 60% | 소유 사용자가 일치하는 seed 기반 주문만 사용한다. |
...
주문 생성은 재고 예약과 outbox side effect를 포함한다. 같은 candidate의 warmup과 measurement에서는 쓰기 주소를 재사용하지 않으며, 다음 독립 candidate 전에는 Order DB를 같은 seed로 다시 시드한다. Order만 선택해도 취소 이벤트를 소비하는 Payment의 `payment-refund-consumer`를 필수 downstream으로 함께 배포하고, `dropmong-payment` 자원과 Kafka lag를 같은 시행 자료에 포함한다.
...
Payment는 주문 하나에 terminal payment 하나만 허용한다. 짧은 시행이라도 동일 주문을 approve와 fail에 동시에 쓰지 않으며, 주소 범위가 부족하면 임의 재사용 대신 실패한다.
...
- 인증이 필요한 외부 Istio 요청은 seed에서 계산한 `userId`로 로그인해 k6 메모리에서만 보관한 Bearer session을 사용한다. `X-Principal`과 `X-User-Id`로 인증을 우회하지 않는다.
- email sign-in 비밀번호, password hash, access token, refresh token, 쿠키는 k6 JSON, Loki, Markdown 보고서에 기록하지 않는다.
- 인증 값은 Kubernetes Secret으로 주입하지 않는다. k6 `setup()`이 정상 로그인으로 token을 얻고 결과 작성 전에 폐기한다.
- Coupon code 원문과 hash key는 loadtest에 전달하지 않으며 결과물에도 넣지 않는다.
