...
const INTEGER_KEYS = [
  ...
  'couponClaimHeadroom', 'couponCodeCount', 'fixturePoolSize', 'fixtureTrialCount',
...
export function loadProfile(document) {
  ...
  const profile = {
    ...
    couponCodeCount: positiveInteger(document, 'couponCodeCount'), fixturePoolSize: positiveInteger(document, 'fixturePoolSize'),
    fixtureTrialCount: positiveInteger(document, 'fixtureTrialCount'), tiers,
  ...
  if ([...Array(profile.dropCount).keys()].some((index) => profile.interestsForDrop(index) > profile.userCount)) throw new TypeError('a drop cannot have more unique interests than users');
  const largestCampaignPool = Math.ceil((profile.userCouponCount + profile.couponClaimHeadroom + profile.couponCodeCount) / profile.couponCampaignCount);
  if (largestCampaignPool > profile.userCount) throw new TypeError('coupon issue, claim, and code pools exceed unique users per campaign');
  return profile;
}
...
export function expectedTableCounts(profile) {
  ...
  return {
    ...
    'order-service': { orders: profile.totalOrderCount, inventory_items: profile.productCount },
    ...
    'coupon-service': { coupon_campaigns: campaigns, coupon_campaign_policy_versions: campaigns, coupon_benefits: campaigns, coupon_applicability_policies: campaigns, coupon_code_batches: campaigns, coupon_codes: profile.couponCodeCount, coupon_issue_requests: profile.userCouponCount, user_coupons: profile.userCouponCount, coupon_redemptions: profile.couponRedemptionCount, rm_user_coupon_wallet: profile.userCouponCount, rm_coupon_details: profile.userCouponCount },
  };
}
