...
const INVARIANTS = {
  'auth-service': [
    ...
    ['pending_outbox', "SELECT count(*)::bigint AS count FROM auth_outbox_events WHERE publish_status <> 'published'"],
  ],
  ...
  'order-service': [['inventory_inconsistent', 'SELECT count(*)::bigint AS count FROM inventory_items WHERE reserved_quantity + sold_quantity > total_quantity'], ['pending_outbox', 'SELECT count(*)::bigint AS count FROM outbox_events WHERE published_at IS NULL']],
  'payment-service': [['payments_without_orders', 'SELECT count(*)::bigint AS count FROM payments p LEFT JOIN known_orders o ON o.order_id=p.order_id WHERE o.order_id IS NULL OR o.user_id <> p.user_id OR o.amount <> p.amount'], ['duplicate_terminal_payment', 'SELECT count(*)::bigint AS count FROM (SELECT order_id FROM payments GROUP BY order_id HAVING count(*) > 1) d'], ['pending_outbox', 'SELECT count(*)::bigint AS count FROM outbox_events WHERE published_at IS NULL']],
  ...
  'coupon-service': [['coupon_issue_graph', 'SELECT count(*)::bigint AS count FROM coupon_issue_requests r LEFT JOIN user_coupons c ON c.issue_request_id=r.issue_request_id LEFT JOIN rm_user_coupon_wallet w ON w.user_coupon_id=c.user_coupon_id AND w.user_id=c.user_id LEFT JOIN rm_coupon_details d ON d.user_coupon_id=c.user_coupon_id AND d.user_id=c.user_id WHERE c.user_coupon_id IS NULL OR w.user_coupon_id IS NULL OR d.user_coupon_id IS NULL'], ['redemption_without_coupon', 'SELECT count(*)::bigint AS count FROM coupon_redemptions r LEFT JOIN user_coupons c ON c.user_coupon_id=r.user_coupon_id WHERE c.user_coupon_id IS NULL'], ['pending_outbox', "SELECT count(*)::bigint AS count FROM domain_outbox WHERE publish_status <> 'published'"]],
};
