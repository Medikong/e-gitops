...
이 디렉터리는 이미 배포된 DropMong dev release를 대상으로 용량과 병목을 측정한다. 서비스 배포, migration, 운영 Secret, 서비스 환경 변수는 만들거나 바꾸지 않는다. 로컬 RUN은 Dataset Job의 직접 DB 입력과 loadtest 전용 이미지만 준비한다.
...
| dataset | 허용된 테이블의 데이터 규모·seed·결정론적 주소 계약 보관 | 서비스 배포, 운영 Secret 준비, 이미지 준비 |
...
| k6 Job | seed에서 API 대상 ID를 재계산한 HTTP 요청과 endpoint별 HTTP 원본 생성 | CPU·메모리 판정 |
...
- `scenario`는 대상 서비스의 순서, `workloads/<service>.js`가 지켜야 할 API/주소 계약, static의 SLO 또는 ramp의 reference 규칙, 최종 보고서 형식을 가진다. 90일 ramp는 `dropmong-web`을 제외한 8개 backend 서비스를 대상으로 한다.
...
- `dataset`은 데이터 양과 결정론적 주소의 단일 원본이다. scenario의 데이터 내용을 복제하지 않는다.
- `environment`는 기존 dev Helm layered values와 namespace, Dataset Job의 직접 DB 입력 참조와 관측성 연결을 가리킨다.
- RUN의 `deployment.replicas`는 실제 적용할 replica 수다. preset의 replica 조건과 맞아야 하며, RUN은 어떤 조건을 어느 환경에서 실행할지 고른다.

`task loadtest:run`은 `values/local.yaml`에 정의한 Dataset Job 환경변수와 loadtest 전용 이미지를 사용한다. Dataset Job은 이 환경변수로 직접 bulk COPY를 수행한다. k6 인증 token은 Job `setup()` 메모리에만 발급한다.
...
task loadtest:run RUN=local-baseline-90days-ramp-replicas-1 SERVICE=catalog-service
```
...
90일 병목 탐색은 다음 선언을 함께 사용한다. `local-baseline-90days-ramp-replicas-1` RUN은 이 세 파일을 조합할 뿐, RPS나 endpoint 규칙을 다시 적지 않는다.

- dataset: `values/datasets/baseline-90days.yaml` - 90일 데이터와 결정론적 주소 규칙
- preset: `values/presets/service-bottleneck-ramp-load-test/baseline-90days-replicas-1.yaml` - 1 replica, 서비스별 연속 ramp 조건
- RUN: `values/runs/local-baseline-90days-ramp-replicas-1.yaml` - 로컬 Istio 환경에서의 실제 조합

실행 전에는 기존 로컬 dev cluster가 준비되어 있어야 한다. 준비가 끝난 뒤 전체 서비스는 다음처럼 순차 실행한다.

```bash
task loadtest:run RUN=local-baseline-90days-ramp-replicas-1 SERVICE=all
```

기존 `local-bottleneck-ramp-replicas-1`은 같은 조합을 가리키는 호환 RUN으로 남겨 둔다.
...
Dataset Job과 k6는 같은 profile·seed에서 사용자·주문·쿠폰·알림 ID를 각각 계산한다. 그래서 대형 manifest 파일이나 별도 데이터 목록 읽기가 없다. 인증이 필요한 workload는 Dataset Job이 만든 결정론적 계정으로 정상 로그인하고, 받은 token은 k6 `setup()` 메모리에만 둔다. 쿠폰 코드와 hash key는 Dataset·k6 경로에 없다. Dataset Job의 DB URL은 `values/local.yaml`의 직접 환경변수로 전달하며, 결과물에는 남기지 않는다.
...
`tmp/datasets/<cache-key>/`의 로컬 binary snapshot cache는 bulk 데이터 생성과 PostgreSQL COPY 시간을 줄이기 위해 유지한다. cache key는 dataset 조건, generator, 관련 schema/migration 식별자와 PostgreSQL 호환 조건을 묶는다. cache hit라도 cache metadata, checksum, schema와 row 계약을 확인한 뒤에만 복원한다.

cache는 서비스 배포나 이미지 준비를 대신하지 않으며 Secret을 보관하지 않는다. 비밀번호, token, cookie, Authorization 값, coupon 평문은 cache metadata, artifact, `result.json`, 로그에 쓰지 않는다.
...
목표는 한계 처리량 탐색이 아니라 Dataset Job, 결정론적 주소, endpoint별 k6 결과, observability 병합, report 생성을 확인하는 것이다. 1일 데이터와 매우 낮은 RPS, 짧은 warmup·측정 시간을 사용한다.
