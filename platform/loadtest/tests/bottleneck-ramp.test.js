...
import { devReleaseContract, fixtureConfigurationFailures, replicaApplyArgs, replicaRestoreArgs } from '../lib/dev-rollout.js';
...
function breachedWindow(index, targetRps, code = 'p95_slo_exceeded') {
...
test('ramp는 Dataset/k6 입력 참조가 없으면 값 생성 없이 configuration 실패로 분류한다', () => {
  const loaded = loadRun(runPath);
  const experiment = { ...loaded, environment: { ...loaded.environment, loadtestInputs: {} } };
  const failures = fixtureConfigurationFailures(experiment, ['catalog-service', 'auth-service', 'coupon-service']);
  assert.deepEqual(failures.map((failure) => failure.service), ['catalog-service', 'auth-service', 'coupon-service']);
  assert.ok(failures.every((failure) => failure.category === 'configuration'));
  assert.doesNotMatch(JSON.stringify(failures), /password|token|coupon-code/i);
});
...
test('최근 window 경계만 집계하고 최소 표본 미달 window는 판정에서 제외한다', () => {
  ...
  const windows = evaluateCompletedWindows(points, {
    ...
    slo: { actualRpsTolerance: 1, errorRate: 0.01, checkPassRate: 1, p95Ms: 100, p99Ms: 200 },
  });
...
test('병목 도달에 따른 k6 중단과 k6 script 실행 실패를 구분한다', () => {
  ...
  assert.equal(rampExitIsExecutionFailure(99, false), false);
...
test('ramp 보고서는 마지막 정상 RPS, 최초 저하 RPS, 종료 조건과 dataset 1회 계약을 보존한다', () => {
  const execution = {
    ...
    services: { 'catalog-service': { status: 'bottleneck_reached', workload: 'drop-list-and-detail', replicas: 1, ramp: { status: 'bottleneck_reached', schedule: { startRps: 10, maxRps: 300, increaseRpsPerSecond: 2 }, evaluation_window_seconds: 10, dataset_preparations: 1, windows: [], last_healthy_rps: 40, first_degraded_rps: 50, terminated_at: '2026-07-24T00:00:30Z', stop_condition: { required_windows: 2, reasons: ['p95_slo_exceeded'] }, first_bottleneck_candidate: { candidate: 'api-slo-or-error' } } } },
  };
  ...
  assert.match(renderAnalysis(result), /2개 연속 위반/);
});
