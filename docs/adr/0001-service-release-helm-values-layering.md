...
`order` 서비스만 배포할 때는 서비스 image를 service/release pipeline에서 만들고 registry에 게시한 뒤, GitOps repo에서 `order`의 image tag만 갱신한다.

```text
service repo
-> order-service image build/push
-> gitops repo values/services/order.yaml 또는 values/overrides/<env>/order.yaml image tag update
-> Argo CD order-aws-dev Application sync
-> Kubernetes order Deployment rollout
```
...
```text
apps/order/values/deployment.yaml
apps/order/values/service.yaml
apps/order/values/ingress.yaml
apps/order/values/network-policy.yaml
apps/order/values/hpa.yaml
apps/order/values/pdb.yaml
```
...
- `order`만 sync, rollback, canary 전환하는 흐름을 만들 수 있다.
