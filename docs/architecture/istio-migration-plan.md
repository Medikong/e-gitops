...
- 애플리케이션 ingress에는 public SG/NLB를 추가하지 않는다. 발표용 Grafana만 별도 workload와 `/grafana` route를 선택하는 임시 전용 NLB의 HTTP/80 예외를 사용한다.
- Grafana 팀 공용 AWS URL과 `127.0.0.1:13000` 로컬 SSM 주소의 차이, 로그인·자격 증명 비공유·발표 후 제거 의무는 [monitoring 접근 계약](../../platform/monitoring/README.md#aws-dev-접근-주소)을 따른다.
...
- 승인된 임시 Grafana `/grafana` 예외 외에 Kiali, DB, admin port와 애플리케이션 route는 외부 비공개를 유지한다.
...
- 임시 Grafana 예외의 로그인·경로 격리·정리 계약이 검증되고, 그 밖의 관리 UI와 데이터 port의 외부 노출이 0이다.
