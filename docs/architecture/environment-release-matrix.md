...
- 애플리케이션 cutover용 public SG, NLB, DNS, TLS는 이 작업에서 새로 만들지 않는다. 발표용 Grafana만 애플리케이션 ingress와 분리된 임시 전용 NLB의 HTTP/80 예외를 사용한다.
- 팀 공용 AWS Grafana 주소는 [monitoring 접근 계약](../../platform/monitoring/README.md#aws-dev-접근-주소)에 기록한다. `127.0.0.1:13000`은 각 사용자 PC의 SSM tunnel 로컬 주소이며 공용 endpoint가 아니다.
- 전용 NLB는 `/grafana`만 전달하고 로그인을 요구한다. 관리자 자격 증명은 Git, 문서, 메신저에 기록하거나 공유하지 않는다.
- 발표 종료 후 public listener/NLB 제거 또는 승인된 팀 CIDR 제한을 확인해야 한다. Kiali, DB, admin port와 애플리케이션 route는 계속 외부 비공개다.
